...
export default class DeleteRegM extends React.Component{
    ...
    handleUpdate(e){
        e.preventDefault();
        Api.delete('org.bitpoll.net.Voter/'+this.state.form.id, {headers: {'Content-Type': 'application/json'}, withCredentials: true}).then(res => {
            alert('Successful');
      ...
      console.log('form 2',this.state.form);
        ...
        return(
          ...
          <Row form>
            <Col md={6}>
              <FormGroup>
                <Label for="countytb">Gender</Label>
                <Input type="text" name="address" id="address" value= {this.state.form.gender}  disabled/>
              </FormGroup>
            </Col>
            <Col md={6}>
              <FormGroup>
                <Label for="countytb">Nationality</Label>
                <Input type="text" name="address" id="nationality" value= {this.state.form.nationality}  disabled/>
              </FormGroup>
            </Col>
          </Row>
          <Row form>
            <Col md={6}>
              <FormGroup>
                <Label for="id">County</Label>
                <Input type="text" name="county" id="id" placeholder="id" disabled value= {this.state.form.county} />
              </FormGroup>
            </Col>
            <Col md={6}>
              <FormGroup>
                <Label for="id">Date of Birth</Label>
                <Input type="text" name="county" id="id" placeholder="id" disabled value= {this.state.form.dob} />
              </FormGroup>
            </Col>
          </Row>       
